...
const Watchlist = () => {
  ...
  //Genres state is to get unique generes from movies so that we can create buttons for particualar genre 
  ...
  useEffect(() => {
    // We're not passing anything from parent component, we're getting data for watchlist from our localStorage
    let watchlistFromLocalStorage = JSON.parse(
      localStorage.getItem("imdb") || "[]"
    );
    //and we're maintaining a new state for this component from localstorage data
    setFavourites(watchlistFromLocalStorage);
  }, []);


  //This UseEffect adds dependency of [Favourites], it means whenver Data ads or Delete/ Changes happen in Favourite/watchlist page 
  //this useEffect will retrigger itself
  useEffect(() => {
    ...
    //Set() function Eliminates Duplicate values, and gives list of unique ones
